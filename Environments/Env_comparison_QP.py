...
class Crosswalk_comparison_QP(gym.Env):
    ...
    def __init__(self, dt, Vm, tau, lower_bounds, upper_bounds, simulation="unif"):
        super(Crosswalk_comparison_QP, self).__init__()
        self.seed()
        self.viewer = False
        self.window = None
    ...
    def new_car(self, action_acc):
        acc = action_acc#self.acceleration(action_acc)
        if math.isnan(acc):
            print("Acc is Nan")
    ...
    def new_reward_meng(self, acc, pos, speed, pos_p, speed_p):
        ...
        # Speed_reward
        rew2 = -2.5 * max(speed - self.Vc*1.1, 0.0) + 1.0 * min(speed, 0.0) + 2.5 * (abs(self.state[1] - speed) < 0.5)
